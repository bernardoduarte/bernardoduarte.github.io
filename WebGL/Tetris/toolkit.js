...
mat4.getCopy = function(matrix){
	var copy = new glArrayType(16);
	copy[0] = matrix[0];
	copy[1] = matrix[1];
	copy[2] = matrix[2];
	copy[3] = matrix[3];
...
mat4.setTranspose = function(matrix){
...
mat4.glScale = function(matrix, scales){
	matrix[0] *= scales[0];
	matrix[1] *= scales[0];
	matrix[2] *= scales[0];
	matrix[3] *= scales[0];
...
mat4.getModelMatrix = function(scales, rotations, translations){
	//TODO Scales
	var matrix = mat4.getIdentity();
	mat4.rotateX(matrix, rotations[0]);
	mat4.rotateY(matrix, rotations[1]);
	mat4.rotateZ(matrix, rotations[2]);
	mat4.translateVector(matrix, translations);
	return matrix;
};
